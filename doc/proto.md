...
The following primitives are used:
  ...
  * BLAKE2s, [RFC7693](https://datatracker.ietf.org/doc/html/rfc7693)
...
Both server and client can now calculate a 32 byte authentication code (`response`) by calculating the BLAKE2s hash of the ASCII string `de.n621.pbotp.v2`, a NUL byte and `login_data` with key `dh_secret` (both the key and output lengths are set to 32). `login_data` is considered an opaque blob here, but serves to bind the result of the exchange to a specific device. A possible format of `login_data` is described below.
...
For the "code phrase" mechanism, the 4-word phrase is `correct horse avocado cupboard`.

The maximum-length phrase (23 words) is `correct horse avocado cupboard unlock federal elevator garbage convince boat bitter gesture tray clarify sense rural luxury recall void spoon deputy swap swallow`.
